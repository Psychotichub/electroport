...
app.use(cors({
    ...
    allowedHeaders: ['Content-Type', 'Authorization', 'X-Requested-With', 'X-Site', 'X-Company', 'x-site', 'x-company']
}));
...
// Legacy static routes (kept for compatibility during transition)
...
// Default route now serves React app in production
app.get('/', (req, res) => {
    if (isProd) {
        return res.sendFile(path.join(panelDistPath, 'index.html'));
    }
    return res.redirect('/login');
});

// React Router fallback for production build
if (isProd) {
    app.get('*', (req, res, next) => {
        const apiLike = req.path.startsWith('/api/') || req.path.startsWith('/material-submit') || req.path.startsWith('/daily-reports') || req.path.startsWith('/received') || req.path.startsWith('/total-price');
        if (apiLike) return next();
        return res.sendFile(path.join(panelDistPath, 'index.html'));
    });
}
